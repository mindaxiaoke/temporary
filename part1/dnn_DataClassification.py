...
def LSUV_mlp(learning_rate=0.1, L1_reg=0.00, L2_reg=0.000001, n_epochs=30,
             batch_size=2, n_hidden=20, n_hiddenLayers=1, verbose=True,
             dataset='ToTheanoClassification.mat'):
    """
    Demonstrate stochastic gradient descent optimization for a multilayer
    perceptron
    ...
    train_set_x = f.get('Train_X3')
    train_set_x = numpy.array(train_set_x,dtype='float32')
    train_set_y = f.get('Train_Y')
    train_set_y = numpy.squeeze(numpy.array(train_set_y,dtype='float32'))
    valid_set_x = f.get('Valid_X3')
    valid_set_x = numpy.array(valid_set_x,dtype='float32')
    valid_set_y = f.get('Valid_Y')
    valid_set_y = numpy.squeeze(numpy.array(valid_set_y,dtype='float32'))
    test_set_x = f.get('Test_X3')
    test_set_x = numpy.array(test_set_x,dtype='float32')
    test_set_y = f.get('Test_Y')
    test_set_y = numpy.squeeze(numpy.array(test_set_y,dtype='float32'))
    train_set_x = theano.shared(train_set_x)
    train_set_y = T.cast(theano.shared(train_set_y.T),'int32')
    valid_set_x = theano.shared(valid_set_x)
    valid_set_y = T.cast(theano.shared(valid_set_y.T),'int32')
    test_set_x = theano.shared(test_set_x)
    test_set_y = T.cast(theano.shared(test_set_y.T),'int32')
...
if __name__ == '__main__':
    ...
    scipy.io.savemat('dnnModel_data_Class_Train3.mat', mdict={'tanhLayer0_W':classifier.hiddenLayers[0].W,
