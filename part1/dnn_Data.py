...
def LSUV_mlp(learning_rate=0.001, L1_reg=0.00, L2_reg=0.0005, n_epochs=3000,
             batch_size=1, n_hidden=50, n_hiddenLayers=1, verbose=True,
             dataset='ToTheano.mat'):
    """
    Demonstrate stochastic gradient descent optimization for a multilayer
    perceptron
    ...
    train_set_x = f.get('Train_X1')
    train_set_x = numpy.array(train_set_x)
    train_set_y = f.get('Train_Y')
    train_set_y = numpy.squeeze(numpy.array(train_set_y))
    valid_set_x = f.get('Valid_X1')
    valid_set_x = numpy.array(valid_set_x)
    valid_set_y = f.get('Valid_Y')
    valid_set_y = numpy.squeeze(numpy.array(valid_set_y))
    test_set_x = f.get('Test_X1')
    test_set_x = numpy.array(test_set_x)
    test_set_y = f.get('Test_Y')
    test_set_y = numpy.squeeze(numpy.array(test_set_y))
    ...
    # for output layer
    '''
    w = classifier.outputLayer.W.eval()
    shape = w.shape
    flat_shape = (shape[0],numpy.prod(shape[1:]))
    a = numpy.random.standard_normal(flat_shape)
    u, _, v = numpy.linalg.svd(a, full_matrices=False)
    ...
    while cnt <= i:
        ...
        input = act
        cnt += 1
    act = T.tanh(T.dot(input, classifier.outputLayer.W) + classifier.outputLayer.b)
    var1 = numpy.var(act.eval())
    mean1 = numpy.mean(act.eval())
    print('output layer', 'var = '+ str(var1), 'mean = '+ str(mean1))
    iter_num = 0
    # feedforward mini-batch until convergence or maximum iteration
    while (abs(needed_variance - var1) > margin):
        weights = classifier.hiddenLayers[i].W
    ...
    print('... training')
    # early-stopping parameters
    patience = 2500*n_train_batches  # look as this many examples regardless
...
if __name__ == '__main__':
    ...
    scipy.io.savemat('dnnModel_data1.mat', mdict={'tanhLayer0_W':classifier.hiddenLayers[0].W,
